...
export default {
  ...
  methods: {
    async payment() {
      await this.$store.dispatch("payment", this.orderId);
    },
    async pay() {
      let result = await QRCode.toDataURL(this.paymentInfo.codeUrl);
      this.$alert(`<img src=${result}></img>`, "请您微信支付", {
      ...
      if(!this.timer){
        this.timer=setInterval(async ()=>{
          try {
            await this.$store.dispatch('payStatus',this.orderId)
           clearInterval(this.timer);
           this.timer=null;
           this.payCode= this.code
           this.$msgbox.close()
           this.$router.push('/paysuccess')
          } catch (error) {
